...
| The Avalonia project's package id keeps `.Avalonia`; its assembly and namespaces stay `.AvaloniaUI` | A package id is permanent; a namespace segment `Avalonia` shadows Avalonia's root | `AssemblyQualityTests.AQ1004_no_namespace_segment_shadows_a_referenced_root` |
...
| Every `CancellationToken` parameter is required | A default lets a caller skip cancellation without noticing | `AssemblyQualityTests.AQ1001_no_public_method_takes_a_defaulted_cancellation_token` |
